...
'@harperfast/oauth':
  ...
  cacheDynamicProviders: true   # Cache forever (only if the config never changes at runtime)
```

If your `onResolveProvider` already caches at the lookup layer (e.g. it memoizes its own DB reads and decryption), set `cacheDynamicProviders: false` and let that layer own caching — the plugin will simply call the hook each time.
...
- `oauthUser` - OAuth user profile (username, email, name, role, `providerUserId`).
  - `oauthUser.authEvidence` is the plugin's own trust determination — base account adoption on it, not on `emailVerified`. It is attached only when an `onLogin` hook is registered, and is a frozen login-time snapshot with:
    ...
    - `signatureVerified` / `issuerValidated` — id-token verification outcomes. `issuerValidated` means the `iss` matched _a_ configured issuer; **which** one is `idTokenIssuer`. Both describe the **ID TOKEN only** — they say nothing about `email` or `emailVerified`, which can come from an unsigned UserInfo response (e.g. `fetchEmail: true` with an id token lacking an `email` claim, `preferIdToken: false`, or a custom `getUserInfo` adapter). A custom policy built from the individual fields, rather than `emailAuthenticated` itself, must still require `emailProvenance !== 'unauthenticated'` — `signatureVerified && issuerValidated && emailVerified === true` alone re-creates the spoofable-email adoption bug this evidence exists to close.
    - `idTokenIssuer` / `idTokenSubject` — the validated issuer and subject (both present only when signature+issuer verified). To bind by stable identity, key on the **pair** `(idTokenIssuer, idTokenSubject)` — `idTokenSubject` alone is not unique when a provider config lists multiple issuers. Some providers accept issuer aliases (Google issues `iss` as either `https://accounts.google.com` or `accounts.google.com`) — normalize known aliases before storing or comparing a binding, or the same account can mint two different keys.
    - `emailVerified` (`boolean | undefined`) and `email` (the address the evidence describes — use it, not a later-mutated `oauthUser.email`). `authEvidence.emailVerified` is the same unauthenticated provider claim as `oauthUser.emailVerified` below — not attested by `signatureVerified`/`issuerValidated`.
